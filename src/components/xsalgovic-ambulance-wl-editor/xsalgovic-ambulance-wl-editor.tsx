...
import {
  AmbulanceConditionsApiFactory, AmbulanceWaitingListApiFactory, Condition, WaitingListEntry
} from "../../api/ambulance-wl";

@Component({
  tag: "xsalgovic-ambulance-wl-editor",
  styleUrl: "xsalgovic-ambulance-wl-editor.css",
  shadow: true,
})
export class XsalgovicAmbulanceWlEditor {
  @Prop() entryId: string;
  ...
  private async getWaitingEntryAsync(): Promise<WaitingListEntry> {
    ...
    try {
      const response = await AmbulanceWaitingListApiFactory(
        undefined,
        this.apiBase
      ...
      <Host>
        ...
        <div class="duration-slider">
          ...
          <md-slider
            ...
            oninput={(ev: InputEvent) => {
              if (this.entry) {
                this.entry.estimatedDurationMinutes = Number.parseInt(
